...
L_dist = [1000000, 100000, 50000, 10000, 1500]; 
L_aagtg = ['AAGTG']; 
...
L_nombres_pssm_main = ['Nkx25', 'Tbx20', 'Meis1', 'Tgif1', 'Hand1', 'Maf', 'Gata1', 'Gata6', 'Gata4']; 
...
if ib3:
    ...
    path_pwm_main = path_pwm_casa; 

bed_main = bed_dupays; 
nom_genoma_main = nom_genoma_dupays; 
genoma_main = genoma_dupays; 
modificador_nomout = '_rnaseq_pssm'; 
test_state = False; 

dist_main = L_dist[0]; 
L_dist_main = [1000000, 100000, 50000, 1500]; 
#L_dist_main = [1000000]; 
...
def generar_dict_entrez_ensembl(genome_name, biomart_server='http://uswest.ensembl.org/biomart'):
...
def pipeline_bed(nom_bed, nom_genoma, genoma, dist_max, dict_entrez_ensembl, L_sitios=[], L_pssm=[], L_nombres_pssm=[], nom_out='', nom_rnaseq='', 
    ...
    if nom_rnaseq != '':
        L_genes_rnaseq = seleccionar_genes_rnaseq(nom_rnaseq, genoma, dict_entrez_ensembl, path_rnaseq=path_rnaseq); 
    else:
        L_genes_rnaseq = []; 
    # Proceso M_bed
...
def procesar_bed(M_bed, genome_name, genome, dist_max, L_sitios=[], L_pssm=[], L_nombres_pssm=[], L_col=[0,1,2], L_genes_rnaseq=[], path_fasta='', display_rate=10):
...
def _main():
    ...
    #dict_entrez_ensembl = _test_retry_generar_dict(max_retries=100, path_usado=path_rnaseq_main); 
    dict_entrez_ensembl = cargar_dict('dict_entrez_ensembl', path_dict=path_rnaseq_main); 
    # Defino L_pssm
    ...
    L_col = [0,1,2]; 
    extra_header = ['peak_id', 'peak_score']; 
    # Corro pipeline_bed para varias distancias
    for curr_dist in L_dist_main:
        # Defino variables necesarias para pipeline_bed
        nom_out = 'DupaysClassPeaks_SitiosConf_dist' + str(curr_dist) + modificador_nomout + '_prueba'*test_state; 
        # Corro pipeline_bed
        M_out = pipeline_bed(bed_main, nom_genoma_main, genoma_main, curr_dist, dict_entrez_ensembl, L_sitios=L_sitios_main, L_pssm=L_pssm, nom_out=nom_out, 
                             nom_rnaseq=nombre_rnaseq, L_col=L_col, path_bed=path_bed_main, path_fasta=path_fasta_main, path_out=path_out_main, 
